...
import matarillo.example.data.DataDTO;
import matarillo.example.data.RestClientService;
...
public class LazyDTOView extends Main {
    ...
    public LazyDTOView(@Autowired RestClientService service) {
        ...
        final Button fetchData = new Button("Create lazy provider", e -> {
             ...
             */
            dataGrid
                .setItems(q -> service.fetchData(q.getLimit(), q.getOffset()))
                .setItemCountCallback(q -> service.fetchCount());
        });
